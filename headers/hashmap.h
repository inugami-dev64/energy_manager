 ...
 * Description: Function declarations to create, access and destroy hashmap instaces
 */
...
typedef struct Hashmap {
    ...
    /// key2 is longer than key1 then returns 1 and returns 2 if keys are the same length
    /// but their memory areas do not match
    static int __keycmp(void *key1, size_t n1, void *key2, size_t n2);


    /// Find the element index by given key
    static int64_t __findIndex(Hashmap *p_hm, void *key, size_t key_size);

    
    /// CRC32 value lookup table for 256 values which are used
    /// to calculate crc32_key from each key byte which itself then
    /// would be used to calculate the actual element location in the map
...
/// Destroy and free all resources allocated for given hashmap instance
